...
export class Page {
  ...
  constructor(
    ...
    this.totalElements = isDefined(totalElements)
      ? totalElements
      : this.totalElements;
...
export class ListQuery {
  ...
  resetFilters(): ListQuery {
    return new ListQuery(this.page, [], this.sort);
  }

  setFilters(filters: Filter[]): ListQuery {
    const pageFirst = new Page(
      0,
      this.page.pageSize,
    ...
    return new ListQuery(pageFirst, filters, this.sort);
  }
  ...
  updateFilter(filter: Filter): ListQuery {
    ...
    return new ListQuery(pageFirst, [...filters, filter], this.sort);
  }
  ...
  nextPage(): ListQuery {
    return new ListQuery(
      new Page(this.page.pageNumber + 1, this.page.pageSize),
      this.filters,
      this.sort
    );
  }
...
export const generateDefaultListQuery = (
  sortBy: string,
  orderBy: SortDirection,
  pageNumber = 0,
  pageSize = 10
) => {
  const sort = new Sort(sortBy, orderBy);

  return new ListQuery(new Page(pageNumber, pageSize), [], sort);
};
